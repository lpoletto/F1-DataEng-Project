...
def merge_fact_driver_standings_to_gold(spark):
    
    print("\n################## Step 1 - Read data from gold layer ##################\n")
    fact_driver_standings_df = spark.read.parquet(f"{GOLD_LAYER_PATH}/fact_driver_standings")

    print("\n################## Step 2 - Write data to PostgreSQL ##################\n")
    print("\n################## Saving data to STG Table ##################\n")
    fact_driver_standings_df.write \
    .format("jdbc") \
    .option("url", f"jdbc:postgresql://{DB_HOST}:{DB_PORT}/{F1_DB}") \
    .option("dbtable", f"{STG_SCHEMA}.fact_driver_standings") \
    .option("user", DB_USER) \
    ...
    sql_query = f"""
    CREATE TABLE IF NOT EXISTS {F1_GOLD_SCHEMA}.fact_driver_standings (
        driver_standings_sk BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        driver_standings_id int4 NOT NULL,
        ...
        CONSTRAINT fact_driver_standings_id_uk UNIQUE (driver_standings_id),
    ...
    sql_query = f"""
    SELECT COUNT(*) FROM {F1_GOLD_SCHEMA}.fact_driver_standings WHERE driver_standings_id = -1;
    """
    ...
    if result_query[0][0] == 0:
        sql_query = f"""
        INSERT INTO {F1_GOLD_SCHEMA}.fact_driver_standings(
            driver_standings_id,
            race_id,
            driver_id,
            date_id,
    ...
    sql_query = f"""
    MERGE INTO {F1_GOLD_SCHEMA}.fact_driver_standings as tgt
    USING {STG_SCHEMA}.fact_driver_standings as stg
    ON tgt.driver_standings_id = stg.driver_standings_id
...
if __name__ == "__main__":
    ...
    merge_fact_driver_standings_to_gold(spark)
