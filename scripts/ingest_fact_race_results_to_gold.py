...
def ingest_fact_race_results_to_gold(spark, execution_date):
    ...
    fact_race_results_df.show(2, truncate=False)
    
    print("\n################## Step 2 - Write data to datalake as parquet ##################\n")
    
    spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic") 

    fact_race_results_df.write.mode("overwrite").partitionBy("race_id").parquet(f"{GOLD_LAYER_PATH}/fact_race_results")
    ...
    fact_race_results_df.write \
        .format("jdbc") \
        .option("url", f"jdbc:postgresql://postgres_db:5432/{F1_DB}") \
